...
- **QQ ↔ 游戏账号绑定**：QQ 内直接绑定（飞升填**游戏内商店 ArkShop 里显示的 EOS ID**）、或游戏公屏验证码 `zsbind` 绑定
...
4. **绑定方式**：推荐**直接绑定**——飞升填 `EOS ID`（32 位，**游戏中打开商店 ArkShop 就能看到自己的**），进化填 `SteamID64`（17 位）：`/绑定 飞升 <EOS>`；不想找 ID 就用验证码流程：QQ 里 `/绑定 飞升 开绑` → 游戏公屏 `zsbind <验证码>`；
...
| `status_check_interval_seconds` | **状态探测**独立定时（秒，默认 `5`，下限 1）：只做 RCON 探测并量往返延迟，不重复抓数据源 |
...
| `status_slow_pm_qq` | 卡顿告警与卡顿日报是否**同时私聊 QQ 主人**（`owner_qq`，默认 `false`；失败只记日志，不影响 KOOK 私聊） |
