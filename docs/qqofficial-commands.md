...
```json
{
  ...
  "panel": {
    ...
    "items": [
      ...
      { "type": "command", "name": "菜单", "desc": "编号快捷入口 /1-/6" },
      { "type": "command", "name": "在线玩家", "desc": "在线人数与部落" },
      { "type": "command", "name": "状态", "desc": "各服在线/离线一览" },
      ...
      { "type": "command", "name": "我的ID", "desc": "查身份ID（关联/核对用）" },
      { "type": "command", "name": "查绑定", "desc": "查看我的绑定" },
      { "type": "command", "name": "解绑", "desc": "解除某个游戏绑定" },
      { "type": "command", "name": "关联", "desc": "QQ ↔ KOOK 身份打通" },
...
| 指令名 | 面板说明建议 | 实际用法 |
| --- | --- | --- |
| 帮助 | 查看全部指令 | `帮助` / `/帮助` / `/help`（`/帮助 查询`、`/帮助 绑定`、`/帮助 管理`） |
| 菜单 | 编号快捷入口 | `菜单` / `/menu`，之后 `/1`～`/6` |
| 进化 | 查进化服 | `进化 [地图名]` / `/ase` |
| 飞升 | 查飞升服 | `飞升 [地图名]` / `/asa` |
| 查服 | 按IP或地图名查服 | `查服 <IP:端口 或 地图名> [ASE\|ASA]` / `/ark` |
| 状态 | 各服在线/离线一览 | `状态` / `/serverstatus` |
| 倍率 | 当前动态倍率 | `倍率` / `/rate` |
...
| 我的ID | 查你的身份ID（关联/核对用） | `我的ID` / `/myid` → 直接显示（绑定请用 `绑定 飞升 <游戏内商店 ArkShop 看到的EOS>`） |
| 解绑 | 解除某个游戏绑定 | `解绑 进化` / `解绑 飞升`（换绑＝先解绑再重绑） |
...
> 群内触发：**发 `/指令` 就会响应**（AstrBot 的 `wake_prefix = "/"`），不一定要 @机器人；裸指令（不带 `/`、不 @）不会触发。

## 四、不用登记到平台的指令

| 指令 | 原因 |
| --- | --- |
| `zsbind <验证码>` | **游戏内公屏指令**，由机器人读取游戏聊天库处理，不属于 QQ 侧指令（先在 QQ 里 `/绑定 <游戏> 开绑` 拿 6 位码） |
| `qqbind <身份ID>`（默认停用） | 同上，需在公屏贴 `/我的ID` 那串，已因隐私默认关闭；`game_bind.qq_cmd=true` 可恢复 |
...
3. **群内发 `/指令` 即可**（不必 @机器人）；群设置需开启"获取群内全部消息"与"机器人主动在群聊内发言"；
4. 本机器人下绑定主键是 openid（私聊 `user_openid` / 群 `member_openid`，ZeroARK 实测两者一致）；QQ 个人认证**没有私聊**，需私发的验证码会直接回到群里（配置 `bind_code_public_fallback`）；
5. KOOK 侧的指令面板配置见同目录说明文档（KOOK 开发者后台 → 机器人 → 指令面板）。
