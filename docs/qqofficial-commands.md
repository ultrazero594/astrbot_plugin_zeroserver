...
```json
{
  ...
  "panel": {
    ...
    "items": [
      ...
      { "type": "command", "name": "我的ID", "desc": "查身份ID，游戏内绑定用" },
...
| 我的ID | 查你的身份ID（游戏内绑定用） | `我的ID` / `/myid` → 游戏公屏发 `qqbind <ID>` |
...
| 指令 | 原因 |
| --- | --- |
| `qqbind <身份ID>`、`zsbind <验证码>` | **游戏内公屏指令**，由机器人读取游戏聊天库处理，不属于 QQ 侧指令（身份ID 用 QQ 里的 `/我的ID` 获取） |
